...
class StyleTransfer:
    ...
    def __init__(self, H=256, W=256, C=3, alpha=1e-3, beta=1.0, iteration=500, content_path="./content//content.jpg", style_path="./style//style.jpg"):
        self.content_img = tf.compat.v1.placeholder("float", [1, H, W, C])
        self.style_img = tf.compat.v1.placeholder("float", [1, H, W, C])
        self.target_img = tf.compat.v1.get_variable("target", shape=[1, H, W, C], initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.02))
        ...
        #L-BFGS
        self.optimizer = tf.compat.v1.contrib.opt.ScipyOptimizerInterface(self.total_loss, method='L-BFGS-B',options={'maxiter': iteration, 'disp': 0})
...
if __name__ == "__main__":
    st = StyleTransfer(H=512, W=512, C=3, alpha=1e-5, beta=1.0, iteration=500, content_path="content.jpg", style_path="style.jpg")